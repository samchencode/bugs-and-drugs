...
describe('AntibiogramGroupController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new AntibiogramGroupController(action);
  });
  ...
  it('should create view model based on fake data', () =>
    controller.index().then((result) => {
      ...
      expect(result[0].intervals[0].groups[0].title).toBe('Antibiogram');
      expect(result[1].intervals[0].groups[0].title).toBe('Antibiogram');
