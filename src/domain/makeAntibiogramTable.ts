...
import type { AntibioticValue, OrganismValue } from '@/domain/Antibiogram';
...
class AntibiogramTableBuilder {
  ...
  makeLabels({ antibiotics, organisms }: Antibiogram) {
    this.#labels = {
      rows: organisms.map((o) => this.#makeOrganismLabel(o)),
      columns: antibiotics.map((a) => this.#makeAntibioticLabel(a)),
    };
  }
